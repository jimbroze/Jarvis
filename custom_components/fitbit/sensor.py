...
def request_app_setup(
    ...
    start_url = f"{get_url(hass, require_ssl=True, prefer_external=True)}{FITBIT_AUTH_CALLBACK_PATH}"
...
def request_oauth_completion(hass: HomeAssistant) -> None:
    ...
    start_url = (
        f"{get_url(hass, require_ssl=True, prefer_external=True)}{FITBIT_AUTH_START}"
    )
...
def setup_platform(
    ...
    if (
        ...
        redirect_uri = f"{get_url(hass, require_ssl=True, prefer_external=True)}{FITBIT_AUTH_CALLBACK_PATH}"
...
class FitbitAuthCallbackView(HomeAssistantView):
    ...
    async def get(self, request: Request) -> str:
        ...
        if data.get("code") is not None:
            redirect_uri = f"{get_url(hass, require_ssl=True, prefer_external=True, require_current_request=True)}{FITBIT_AUTH_CALLBACK_PATH}"
